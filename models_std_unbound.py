...
def get_gaussian_filter(kernel_size=3, sigma=2, channels=3):
  ...
  print("Gaussian Kernel Shape: ", gaussian_kernel.size())
  print(gaussian_kernel)

  # Reshape to 2d depthwise convolutional weight
  gaussian_kernel = gaussian_kernel.view(1, 1, kernel_size, kernel_size)
  gaussian_kernel = gaussian_kernel.repeat(channels, 1, 1, 1).cuda()
  #print("Final Gaussian Kernel Shape: ", gaussian_kernel.size())
  if kernel_size == 3:
    padding = 1
  else:
    padding = 0
...
class CNNNormal(nn.Module):
  def __init__(self, nc, num_classes,precision_point,dataset, std=1):
    ...
    CSVFile = "CNN_CBS_"+ self.dataset+"_Precision_Point_" + precision_point + ".csv"
  ...
  def get_new_kernels(self, epoch_count):
      #if epoch_count % 10 == 0:
        #self.std *= 0.925
      self.kernel0 = get_gaussian_filter(kernel_size=3, sigma=self.std0 / 1, channels=3)
      self.kernel1 = get_gaussian_filter(kernel_size=3, sigma=self.std1 / 1, channels=32)
      self.kernel2 = get_gaussian_filter(kernel_size=3, sigma=self.std2 / 1, channels=64)
      self.kernel3 = get_gaussian_filter(kernel_size=3, sigma=self.std3 / 1, channels=128)
      self.kernel4 = get_gaussian_filter(kernel_size=3, sigma=self.std4 / 1, channels=256)
