...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, args):
        ...
        CSVFile = "ResNet18_CBS_"+ self.dataset+"_Precision_Point_" + precision_point + ".csv"
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
         y = x1.float()
         self.std1 = torch.std(y)
        print("Outside Basic block")
    ...
    def get_new_kernels(self, epoch_count):
        ...
        for child in self.layer1.children():
            print("child 1")
            child.get_new_kernels(self.kernel_size, self.std0)
