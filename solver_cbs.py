...
class CBSSolver(BaseSolver):
    ...
    def trainValError(self,alg):
        column_name = ['Epoch Number', 'Cross Entropy Loss', 'Validation Error']
        directory = "CSV_Data/trainValData/"
        CSVFile = alg + "_trainVal" + ".csv"
    ...
    def solve(self):
        ...
        for epoch_count in range(self.args.num_epochs):
            ...
            if epoch_count % 1 == 0:
                ...
                cprint('epoch count: {}'.format(epoch_count), "red", "on_white")
            ...
            row = [epoch_count,loss.item() ,(100-accuracy)] #Write epoch, train error and validation error on CSV file
            with open(train_valFile, mode='a') as write_obj:
                csvwriter = csv.writer(write_obj)
