...
class VGG16_conv(torch.nn.Module):
    def __init__(self, n_classes, args):
        ...
        CSVFile = "VGG16_CBS_"+self.dataset+"_Precision_Point_"+precision_point+".csv"
        self.cfile = directory + CSVFile
        current_working_dir = os.getcwd()
        filename = os.path.join(current_working_dir, self.cfile)
        print(filename)
        header = ['Data_Std', 'VGG16_CBS_Layer_3_Std', 'VGG16_CBS_Layer_6_Std', 'VGG16_CBS_Layer_9_Std', 'VGG16_CBS_Layer_12_Std',
                  'VGG16_CBS_Layer_16_Std', 'Data_Mean', 'VGG16_CBS_Layer_3_Mean', 'VGG16_CBS_Layer_6_Mean', 'VGG16_CBS_Layer_9_Mean',
                  'VGG16_CBS_Layer_12_Mean', 'VGG16_CBS_Layer_16_Mean','VGG16_Std']
    ...
    def forward(self, x, return_intermediate=False):
        ...
        with torch.no_grad():
          y = x1.float()
          mean1 = torch.mean(y).item()
          std1 = torch.std(y).item()
        x1 = self.kernel1(x1)
        x1 = self.post1(x1)
        ...
        with torch.no_grad():
          y = x2.float()
          mean2 = torch.mean(y).item()
          std2 = torch.std(y).item()
        x2 = self.kernel2(x2)
        x2 = self.post2(x2)
        ...
        with torch.no_grad():
          y = x3.float()
          mean3 = torch.mean(y).item()
          std3 = torch.std(y).item()
        x3 = self.kernel3(x3)
        x3 = self.post3(x3)
        ...
        with torch.no_grad():
          y = x4.float()
          mean4 = torch.mean(y).item()
          std4 = torch.std(y).item()
        x4 = self.kernel4(x4)
        x4 = self.post4(x4)
        ...
        with torch.no_grad():
          y = x5.float()
          mean5 = torch.mean(y).item()
          std5 = torch.std(y).item()
        x5 = self.kernel5(x5)
